...
initialize_starting_pos_grid(I,J,N):-
	...
	((I =:= N/2, J =:= N/2, !, Val = 2)			% 2 = o
	;
	(I =:= N/2, J =:= (N/2)+1, !, Val = 1)		% 1 = x
	;
	(I =:= (N/2)+1, J =:= N/2, !, Val = 1)		% 1 = x
	;
	(I =:= (N/2)+1, J =:= (N/2)+1, !, Val = 2) 	% 2 = o
	;
...
run:-
	...
	((Mode =< 2, get_game_level(Level), play_interactive_game(Count,Mode,Level,pos(0,1,_))	
	 ;
	 % watch automatic game computer vs computer 
	 Mode =:= 3, get_game_level(Level), get_game_level2(Level2), play_automatic_game(Count,Level,Level2,pos(0,1,_))
	;
	 % get the result of the match between 2 computers with different heuristic 
	 Mode =:= 4, get_game_level(Level), get_game_level2(Level2), compare_heuristic(0,0,0,0,0,Level,Level2))
	;
	% user quit game explictly 
	(user_exited_game,!, print_goodbye_message(PlayerName), cleanup)
	;
	% game finished normally 
	(Mode < 4, end_of_game(FinalGrid),
...
play_automatic_game(Count,Level,Level2,pos(Grid1,Computer1,_)):-
	...
	nl,write('Computer'),write(Computer1),write(' plays ('),write(I2),write(','),write(J2),write(').'),
	...
	;
	% in case current player has no legal move, alternate turn 
...
/*******************************************************************************
* Automatic game AI vs AI                                                 
* compare_heuristic(+Level,+Level2)
* play 20 matchs of 2 with given position, each side get 1 white and 1 black
* only the result will be printed
********************************************************************************/
play_automatic_game_fast(P1Win,P2Win,Draw,ResP1Win,ResP2Win,ResDraw,Level,Level2,pos(Grid1,Computer1,_),P1First):-
	% incase current player has a legal move
	get_legal_coordinates(Grid1,Computer1,_),!,
	get_max_depth(Level,MaxDepth),
	alphabeta(pos(Grid1,Computer1,_),_,_,Pos2,_,0,MaxDepth,Level), % get best move 
	Pos2 = pos(Grid2,Computer2,coordinate(I2,J2)),
	print_grid(Grid2),sleep(0.75),
	play_automatic_game_fast(P1Win,P2Win,Draw,ResP1Win,ResP2Win,ResDraw,Level,Level2, pos(Grid2,Computer2,_),P1First)	% alternate turn
	;
	% incase current player has no legal move, alternate turn 
	get_other_player(Computer1,Computer2),
	get_legal_coordinates(Grid1,Computer2,_),!,
	play_automatic_game_fast(P1Win,P2Win,Draw,ResP1Win,ResP2Win,ResDraw,Level,Level2, pos(Grid1,Computer2,_),P1First)
	;
	% both player have no legal move - end game 
	(end_of_game(Grid1)),!,get_game_results(P1Win,P2Win,Draw,ResP1Win,ResP2Win,ResDraw,Grid1,P1First),retractall(end_of_game(_)).
	
compare_heuristic(I,End,P1Win,P2Win,Draw,Level,Level2):-
	play_automatic_game_fast(P1Win,P2Win,Draw,ResP1Win,ResP2Win,ResDraw,Level,Level2,pos(0,1,_),1).
	%write(ResP1Win),write(ResP2Win),write(ResDraw).
	%play_automatic_game_fast(ResP1Win,ResP2Win,ResDraw,Res2P1Win,Res2P2Win,Res2Draw,Level2,Level,pos(0,2,_),0),
	%writeln(Res2P1Win),writeln(Res2P2Win),writeln(Res2Draw).
/*compare_heuristic(I,End,P1Win,P2Win,Draw,Level,Level2):-
	(I > End, write("\nP1Win = "),write(P1Win),write("\nP2Win = "),write(P2Win),write("\nDraw = "),write(Draw))
	;
      play_automatic_game_fast(P1Win,P2Win,Draw,ResP1Win,ResP2Win,ResDraw,Level,Level2,pos(I,Computer1,_),1)
	, play_automatic_game_fast(ResP1Win,ResP2Win,ResDraw,Res2P1Win,Res2P2Win,Res2Draw,Level2,Level,pos(I,Computer1,_),0)
	, I1 is I+1, compare_heuristic(I1,End,Res2P1Win,Res2P2Win,Res2Draw,Level,Level2).*/
...
/* get_board_dimension(+N) */ 
get_board_dimension(N):-
	write('Let''s set the game''s board dimension - '),nl, 
	repeat, 
...
get_game_mode(M):-
	...
	write('3 = Automatic game between me and my self (AI vs AI)'),nl,
	write('4 = 20 matches of 2 between me and myself (AI vs AI)'),nl,
	get_user_input(M),
	((integer(M), M >= 1, M =< 4, !)	% validate input	
	 ;
...
/* get_game_results(+Mode,+FinalGrid,+PlayerName)      */
get_game_results(P1Win,P2Win,Draw,ResP1Win,ResP2Win,ResDraw,FinalGrid,P1First):-
	% get pices statistics 
	pieces_count_evaluation(FinalGrid,_,MaxCount,MinCount),
	% print_grid(FinalGrid),
	% result
	((MaxCount =:= MinCount,!, ResDraw is Draw+1, ResP1Win is P1Win, ResP2Win is P2Win)
	;
	(MaxCount > MinCount,!,P1First =:=1,!, ResP1Win is P1Win+1, ResP2Win is P2Win, ResDraw is Draw)
	;
	(MaxCount > MinCount,!,P1First =:=0,!, ResP2Win is P2Win+1, ResP1Win is P1Win, ResDraw is Draw)
	;
	(MaxCount < MinCount,!,P1First =:=1,!, ResP2Win is P2Win+1, ResP1Win is P1Win, ResDraw is Draw)
	;
	(MaxCount < MinCount,!,P1First =:=0,!, ResP1Win is P1Win+1, ResP2Win is P2Win, ResDraw is Draw)).
